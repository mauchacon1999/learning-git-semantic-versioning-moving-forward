# Git Tag Automation Script (Simplified)
# Autor: Senior Developer
# Descripcion: Automatizacion de tags con versionado semantico para flujo Git Flow
...
function Test-GitRepository {
    if (-not (Test-Path ".git")) {
        Write-ColorOutput "Error: No se encontro un repositorio Git en el directorio actual" "Error"
        exit 1
    }
}
...
function Test-GitStatus {
    ...
    if ($status -and -not $Force) {
        Write-ColorOutput "Advertencia: Hay cambios sin commitear en el repositorio" "Warning"
...
function Get-LastTag {
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-ColorOutput "No se encontraron tags previos. Iniciando con v1.0.0" "Info"
        return "v0.0.0"
    }
    return $lastTag
}
...
function Parse-Version {
    ...
    Write-ColorOutput "Error: Formato de version invalido: $VersionString" "Error"
    exit 1
}
...
function Generate-Changelog {
    ...
    if (-not $commits) {
        Write-ColorOutput "No hay commits nuevos para incluir en el changelog" "Info"
        return
    }
...
function Update-VersionFile {
    ...
    Write-ColorOutput "Archivo de version actualizado: $VERSION_FILE" "Success"
}
...
function Create-GitTag {
    ...
    if ($DryRun) {
        Write-ColorOutput "DRY RUN: Se crearia el tag '$TagName' con mensaje: $Message" "Info"
        return
    }
...
function Push-Tag {
    ...
    if ($DryRun) {
        Write-ColorOutput "DRY RUN: Se haria push del tag '$TagName'" "Info"
        return
    }
...
function Show-Summary {
    ...
    Write-ColorOutput "`nRESUMEN DE LA OPERACION" "Info"
    Write-ColorOutput "================================" "Info"
    Write-ColorOutput "Version anterior: $PreviousVersion" "Info"
...
function Main {
    ...
    Write-ColorOutput "Ultimo tag: $lastTag" "Info"
    
    # Determinar nueva version
    if ($CustomVersion) {
        if ($CustomVersion -notmatch "^v?\d+\.\d+\.\d+$") {
            Write-ColorOutput "Error: Formato de version personalizada invalido. Use formato: X.Y.Z" "Error"
            exit 1
        }
    ...
    # Mostrar informacion antes de proceder
    Write-ColorOutput "`nInformacion de la nueva version:" "Info"
    Write-ColorOutput "   Nueva version: $newVersion" "Success"
    ...
    Write-ColorOutput "`nProceso completado exitosamente!" "Success"
}
